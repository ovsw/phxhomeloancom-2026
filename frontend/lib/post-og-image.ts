...
export const POST_OG_IMAGE_VERSION = "1";
...
export function isValidPostOgSlug(slug: string) {
  return (
    slug.length > 0 &&
...
export function signPostOgImage(
  ...
  return createHmac("sha256", secret)
    .update(signaturePayload(identity))
    .digest("base64url");
}
...
export function verifyPostOgImageSignature({
  ...
  const expected = Buffer.from(signPostOgImage(identity, secret), "base64url");
...
export function getPostOgImageSecret() {
